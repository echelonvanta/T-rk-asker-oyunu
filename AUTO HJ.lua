...
-- Sayıyı Türkçe kelimelere çevirme fonksiyonu (1'den 1000'e kadar, tüm harfler büyük)
...
minimizeButton.BackgroundColor3 = Color3.fromRGB(25, 40, 60)
minimizeButton.Text = "-"
...
-- Mesaj Gönderme ve Zıplama Mantığı (Harf harf ve tam sayı)
local function sendMessageAndJump(index)
    ...
    for i = 1, #msg do
        ...
        if char ~= " " then -- Boşlukları atla
            if chatChannel then
                chatChannel:SendAsync(char)
            else
                ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(char, "All")
            end
            if humanoid then
                humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
            end
            wait(2) -- Her harf için 2 saniye gecikme
        end
    end

    -- Tam sayıyı yaz ve zıpla
    if stopRequested then
        isRunning = false
        currentIndex = 1
        return
    ...
    wait(2) -- Tam sayı için 2 saniye gecikme
    if isRunning then
